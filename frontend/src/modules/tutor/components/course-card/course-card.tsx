...
const CourseCard: React.FC<CombinedCourseCardProps> = (props) => {
  ...
  const isLegacyProps = (props: CombinedCourseCardProps): props is CourseCardProps => {
    return 'openApplyModal' in props;
  };

  const isEnhancedProps = (props: CombinedCourseCardProps): props is EnhancedCourseCardProps => {
  ...
  const getStatusInfo = () => {
    ...
    return {
      label: availability,
      bgClass: availability === "Full Time" ? styles.bgBlue100 : styles.bgPurple100,
      textClass: availability === "Full Time" ? styles.textBlue800 : styles.textPurple800,
    };
  };
    ...
    return (
      ...
      >
        ...
        <div className={styles.cardHeader}>
          <span className={styles.courseCode}>{(course as Course).courseCode}</span>
          <span className={styles.semester}>{(course as Course).semester}</span>
        </div>

        {/* Card Body */}
        <div className={styles.cardBody}>
          <h3 className={styles.courseTitle}>{(course as Course).courseName}</h3>
        ...
        <div className={styles.roleSection}>
          ...
          {roles.map((role) => {
            ...
            const maxPositions = role.roleName === "tutor" 
                  ...
                  <div className={styles.roleAction}>
                    {applicationStatus ? (
                      ...
                      <motion.button
                        ...
                        onClick={() => enhancedProps.onApplyForRole(course as Course, role)}
  ...
  return (
    <div className={styles.enhancedCourseCard}>
      ...
      <div className={styles.cardTop}>
        <span className={styles.courseCode}>{(course as CourseDetails).code}</span>
        <span
      ...
      <div className={styles.cardBody}>
        ...
        <div className={styles.roleTag}>
          ...
          <span className={styles.roleName}>{(course as CourseDetails).role}</span>
        </div>
      </div>
