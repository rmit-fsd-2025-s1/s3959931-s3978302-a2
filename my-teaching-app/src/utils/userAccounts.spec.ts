...
describe("User Accounts Utilities", () => {
    ...
    test("getUserByCredentials returns undefined with invalid credentials", () => {
        ...
        const result1 = getUserByCredentials(
            "test@example.com",
            "wrongpassword"
        );
        const result2 = getUserByCredentials(
            "wrong@example.com",
            "password123"
        );
    ...
    test("validateRoleSpecificEmail validates email formats correctly", () => {
        // Test tutor emails
        expect(validateRoleSpecificEmail("valid@tutor.edu.au", "tutor")).toBe(
            true
        );
        expect(validateRoleSpecificEmail("invalid@gmail.com", "tutor")).toBe(
            false
        );
        expect(validateRoleSpecificEmail("test@TUTOR.EDU.AU", "tutor")).toBe(
            true
        ); // Case insensitive

        // Test lecturer emails
        expect(
            validateRoleSpecificEmail("valid@lecturer.edu.au", "lecturer")
        ).toBe(true);
        expect(validateRoleSpecificEmail("invalid@gmail.com", "lecturer")).toBe(
            false
        );
        expect(
            validateRoleSpecificEmail("test@LECTURER.EDU.AU", "lecturer")
